...
The 2016 Index score for `r country_name` is of `r read_csv(file.path(dir_global, '/data/scores_eez2016.csv')) %>% filter(country == country_name) %>% select(Index)`. Ranking 98 out of 221 countries and territories assessed. 
...
### Improving scores
- Wild Caught Fisheries score increased by 3 points and Food Provision improved 2 points

### Declining scores
- Sense of Place decreased by 1 point 
- Tourism and recreation declined by 2 points
- Natural products decreased by 3 points

![OHI 2016 Global Assessment: `r country_name`'s Flower Plot](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/global2016/Reporting/figures/FlowerPlots/flower_Tanzania_2016.png)

### Explanation of `r country_name`'s scores
The overall score for `r country_name` is `r read_csv(file.path(dir_global, '/data/scores_eez2016.csv')) %>% filter(country == country_name) %>% select(Index)`, which is unchanged from 2015. There were sligh increases for some goals and slight decreases for others, which contributes to the balance and unchanged score. The score of 0 for Mariculture reflects no production of farmed seafood per coastal inhabitant.

<font size = "1"> For a detailed summary of Country results visit: http://www.oceanhealthindex.org/region-scores/scores/tanzania
</font>
